...
if bandwidth is not None and elapsed_time is not None \\
   and ($BANDWIDTH_MIN <= bandwidth <= $BANDWIDTH_MAX) \\
   and ($ELAPSED_TIME_MIN <= elapsed_time <= $ELAPSED_TIME_MAX):
    sys.exit(0)
else:
    print(f'Expected Elapsed Time: $ELAPSED_TIME_MIN - $ELAPSED_TIME_MAX')
    print(f'Returned Elapsed Time: {elapsed_time}')
